...
const TableBody = (props) => {
  return (
    <tr>
      <td className={"flag-container display-" + Boolean(props.name)}>
        {" "}
        <img
          src={props.flag}
          className={"flag display-" + Boolean(props.flag)}
          alt=""
        />{" "}
        {props.name}
      </td>
      <td>
        {props.updated === undefined
          ? "Dinosaur"
          : new Date(props.updated).toLocaleDateString()}
      </td>
      ...
      <td className={"display-" + Boolean(props.tests >= 0)}>
        {props.tests || 0}
      </td>
